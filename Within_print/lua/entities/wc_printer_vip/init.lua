...
function ENT:Initialize()
	self:SetModel(hg_printer_vip.model)
	self:SetMaterial(hg_printer_vip.material)
	self:SetColor(hg_printer_vip.color)
	...
	// PRINTER VARS
	self.health = hg_printer_vip.starthealth
	self.money = 0
	self.battery = hg_printer_vip.battery
	self.printtime = 0
	self.battstatus = hg_printer_vip.draintotal
...
function PrinterAction()
	...
	if action == action_battery then
		if printer.battery >= hg_printer_vip.battery then return end
		printer.battery = hg_printer_vip.battery
		ply:addMoney(-hg_printer_vip.newbattery)
		DarkRP.notify(ply, 1, 4, "You bought a new battery for $"..hg_printer_vip.newbattery)
	end
...
function ENT:BatteryModule()
	timer.Create("BatteryTimer"..self:EntIndex(), 1, 0, function()
		...
		self.battery = (self.battery - hg_printer_vip.drainrate)

		if self.printtime >= hg_printer_vip.printtime then
			self:Print()
			self.printtime = 0
		end
...
function ENT:Print()
	if not IsValid(self) then return end
	local rndamount = math.Round(math.Rand(hg_printer_vip.amount, (hg_printer_vip.amount + 20)))
